...
public class ChatMessageAppService : IChatMessageAppService
{
    ...
    public async Task<FamilyMeetMessageDto> SendMessageAsync(SendMessageDto input)
    {
        ...
        var result = _mapper.Map<FamilyMeetMessageDto>(message);
        return result;
    }

    public async Task<FamilyMeetMessageDto> EditMessageAsync(Guid messageId, EditMessageDto input)
    {
    ...
    public async Task<MessagePagedResultDto> GetMessagesAsync(GetMessagesDto input)
    {
        ...
        var messageDtos = _mapper.Map<List<FamilyMeetMessageDto>>(messages);
    ...
    public async Task<FamilyMeetMessageDto> GetMessageAsync(Guid messageId)
    {
        var message = await _messageRepository.GetAsync(messageId);
        if (message == null)
            throw new ArgumentException("Message not found", nameof(messageId));
