...
public class ChatAppService : IChatAppService
{
    ...
    public async Task<FamilyMeetGroupDto> CreateGroupAsync(CreateChatGroupDto input)
    {
        ...
        // Cache the new group
        var result = _mapper.Map<FamilyMeetGroupDto>(group);
        await _cacheService.SetAsync(CacheKeys.ChatGroups(group.Id), result, TimeSpan.FromHours(1));

        return result;
    }

    public async Task<FamilyMeetGroupDto> UpdateGroupAsync(Guid groupId, UpdateChatGroupDto input)
    {
        // Try to get from cache first
        var cachedGroup = await _cacheService.GetAsync<FamilyMeetGroupDto>(CacheKeys.ChatGroups(groupId));
    ...
    public async Task<FamilyMeetGroupDto> GetGroupAsync(Guid groupId)
    {
        // Try to get from cache first
        var cachedGroup = await _cacheService.GetAsync<FamilyMeetGroupDto>(CacheKeys.ChatGroups(groupId));
        if (cachedGroup != null)
    ...
    public async Task<List<FamilyMeetGroupDto>> GetUserGroupsAsync(Guid userId)
    {
        var cacheKey = $"user:groups:{userId}";

        // Try to get from cache first
        var cachedGroups = await _cacheService.GetAsync<List<FamilyMeetGroupDto>>(cacheKey);
        ...
        var result = _mapper.Map<List<FamilyMeetGroupDto>>(groups);
    ...
    public async Task<FamilyMeetGroupDto> JoinGroupAsync(JoinGroupDto input)
    {
